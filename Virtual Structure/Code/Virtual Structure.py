...
def main():
    ...
    prev_xx1, prev_yy1 = 0, 0
    prev_x_dot_d1, prev_y_dot_d1 = 0, 0

    prev_xx2, prev_yy2 = 0, 0
    prev_x_dot_d2, prev_y_dot_d2 = 0, 0

    prev_xx3, prev_yy3 = 0, 0
    ...
    cx12, cy12, cyaw12 = 0.5,0.5,5
    cx13, cy13, cyaw13 = 0.5,0.5,5

    cx21, cy21, cyaw21 = 0.5,0.5,5
    cx23, cy23, cyaw23 = 0.5,0.5,5

    cx31, cy31, cyaw31 = 0.5,0.5,5
    cx32, cy32, cyaw32 = 0.5,0.5,5
    ...
    while T >= time and lastIndex > target_ind + 10:
        ...
        xx_d1 = state.x + p * np.cos(state.yaw)
        yy_d1 = state.y + p * np.sin(state.yaw)
        xx_d2 = state.x + p * np.cos(state.yaw + np.deg2rad(120))
        yy_d2 = state.y + p * np.sin(state.yaw + np.deg2rad(120))
        xx_d3 = state.x + p * np.cos(state.yaw + np.deg2rad(240))
        yy_d3 = state.y + p * np.sin(state.yaw + np.deg2rad(240))
        print("------")
        ...
        v_in1 = v_d1 + (cx1*xe1) - (cy1*omega_d1*ye1) + \
                (cx12*(xe1 - xe2)) + (cx13*(xe1 - xe3)) - \
                ((cy12*omega_d1*(ye1 - ye2)) + (cy13*omega_d1*(ye1 - ye3)) )
        omega_in1 = omega_d1 + (cyaw1*yawe1) + \
                    (cyaw12*(yawe1 - yawe2)) + (cyaw13*(yawe1 - yawe3)) 
